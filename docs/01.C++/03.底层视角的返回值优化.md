...
根据 [system v abi x86-64](https://gitlab.com/x86-psABIs/x86-64-ABI) 的描述，一般来说不超过 16 字节的平凡可复制类型可以用寄存器传递（其实细节上不是这样的，abi 给的描述非常复杂），本文将其称为简单类型。
